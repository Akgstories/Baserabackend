...
def send_email_notification(recipient_email: str, subject: str, body_text: str, html_content: Optional[str] = None):
    ...
    if clean_brevo_key and clean_brevo_key.startswith("xkeysib-"):
        try:
            ...
            req = urllib.request.Request(
                url, data=data,
                headers={"accept": "application/json", "api-key": clean_brevo_key, "content-type": "application/json"},
                method="POST"
            )
    ...
    log_email_event(recipient_email, subject, "Unconfigured", "Missing active email provider configuration.")
...
class DBUser(Base):
    ...
    address: Mapped[str] = mapped_column(String, default="GEC Bokaro Hostel")
...
class DBPGRoom(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, index=True, default=lambda: f"room-{uuid.uuid4().hex[:8]}")
    pg_id: Mapped[Optional[str]] = mapped_column(String, index=True, nullable=True)
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if "pg_rooms" in inspector.get_table_names():
                ...
                if "pg_id" not in cols:
                    conn.execute(text("ALTER TABLE pg_rooms ADD COLUMN pg_id TEXT DEFAULT NULL;"))
                if "id" not in cols:
                    conn.execute(text("ALTER TABLE pg_rooms ADD COLUMN id TEXT DEFAULT NULL;"))
            ...
            if "payment_receipts" in inspector.get_table_names():
                ...
                if "total_amount" not in cols:
                    conn.execute(text("ALTER TABLE payment_receipts ADD COLUMN total_amount FLOAT DEFAULT 0.0;"))
                if "platform_fee" not in cols:
                    conn.execute(text("ALTER TABLE payment_receipts ADD COLUMN platform_fee FLOAT DEFAULT 0.0;"))
                if "vendor_payout_amount" not in cols:
                    conn.execute(text("ALTER TABLE payment_receipts ADD COLUMN vendor_payout_amount FLOAT DEFAULT 0.0;"))

            if "pg_listings" in inspector.get_table_names():
                cols = [c["name"] for c in inspector.get_columns("pg_listings")]
                if "owner_id" not in cols:
                    conn.execute(text("ALTER TABLE pg_listings ADD COLUMN owner_id TEXT DEFAULT NULL;"))
        ...
        if not admin_user:
            db.add(DBUser(
                ...
                password=hash_password("admin@2026"), phone="9155118661", address="Admin Office, GEC Bokaro",
        ...
        if not db.query(DBPGRoom).first():
            db.add_all([
                DBPGRoom(id="room-101", pg_id="pg-1", room_number="101", room_type="Single AC", tenant_name="Rahul Kumar", tenant_phone="9876543210", tenant_address="GEC Bokaro Hostel Block A", monthly_rent=8000, status="occupied", images="[]"),
                DBPGRoom(id="room-102", pg_id="pg-1", room_number="102", room_type="Double Non-AC", tenant_name="-", tenant_phone="-", tenant_address="-", monthly_rent=5200, status="vacant", images="[]")
            ])
...
app = FastAPI(title="Basera Multi-Portal API", version="17.0.0")
...
def notify_owner_and_email(
    ...
    db.add(DBNotification(
        ...
        recipient_id=recipient_user_id, title=title, message=message,
        event_type=event_type, created_at=get_ist_now().strftime("%Y-%m-%d %H:%M")
    ))

    if recipient_user_id:
        target_users = db.query(DBUser).filter(DBUser.id == recipient_user_id).all()
    else:
        target_users = db.query(DBUser).filter(DBUser.role == recipient_role).all()

    for target in target_users:
        if target.email:
            background_tasks.add_task(
                send_email_notification, target.email,
...
def register_user(req: RegisterRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    if role == "admin":
        raise HTTPException(status_code=400, detail="Admin account creation is disabled.")
    ...
    new_user = DBUser(
        ...
        password=hash_password(req.password), phone=clean_phone,
        address=req.address or "GEC Bokaro Hostel", google_map_url=map_url,
    ...
    if role == "student":
        db.add(DBMessStudent(
            id=f"ms-{uuid.uuid4().hex[:6]}", name=new_user.full_name, phone=clean_phone,
            address=new_user.address, google_map_url=map_url, mess_name="Annapurna Homely Mess",
            plan="3-Time Standard Daily Plan", diet="Veg", base_price=3000, is_active=True
        ))

    db.commit()
    background_tasks.add_task(send_email_notification, req.email, "Welcome to Basera Platform!", f"Hello {req.full_name},\n\nYour account has been registered successfully as a {role.upper()} on Basera.")
...
def login_user(req: LoginRequest, db: Session = Depends(get_db)):
    query = db.query(DBUser).filter(DBUser.email == req.email)
    if req.target_role:
    ...
    is_valid, needs_upgrade = verify_and_update_password(req.password, user.password)
    if not is_valid:
...
def update_profile(req: ProfileUpdateRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.commit()
    return {"status": "success", "message": "Profile details updated successfully!"}
...
def forgot_password(req: ForgotPasswordRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    for u in users:
        u.password = hash_password(temp_password)
    db.commit()
    
    background_tasks.add_task(
        send_email_notification, 
        req.email, 
        "Basera Password Reset", 
        f"Your temporary secure password is: {temp_password}\n\nPlease login using this password and update it immediately from your profile settings."
    )
    return {"status": "success", "message": "A temporary secure password has been sent to your registered email."}
...
def get_mess_pricing(mess_id: Optional[str] = Query(None), db: Session = Depends(get_db)):
    ...
    pricing = query.all()
    return [{"id": p.id, "mess_id": p.mess_id, "location_name": p.location_name, "three_time_rate": p.three_time_rate, "two_time_rate": p.two_time_rate} for p in pricing]

@app.post("/api/mess/update-location-price")
def update_mess_location_price(req: UpdateMessLocationPricingRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] not in ["mess_partner", "admin"]:
        raise HTTPException(status_code=403, detail="Unauthorized action.")

    if user["role"] == "mess_partner":
        mess = db.query(DBMessListing).filter(DBMessListing.id == req.mess_id, DBMessListing.provider_name.ilike(f"%{user['full_name']}%")).first()
        if not mess:
            raise HTTPException(status_code=403, detail="Forbidden: You do not own this mess listing.")

    item = db.query(DBMessPricing).filter(DBMessPricing.mess_id == req.mess_id, DBMessPricing.location_name.ilike(req.location_name)).first()
    if not item:
        item = DBMessPricing(mess_id=req.mess_id, location_name=req.location_name, three_time_rate=req.three_time_rate, two_time_rate=req.two_time_rate)
        db.add(item)
    else:
...
def get_mess_listings(db: Session = Depends(get_db)):
    return [{"id": m.id, "name": m.name, "provider_name": m.provider_name, "monthly_price": m.monthly_price, "diet_type": m.diet_type, "meals_per_day": m.meals_per_day, "rating": m.rating, "address": m.address, "google_map_url": m.google_map_url, "description": m.description} for m in db.query(DBMessListing).all()]

@app.post("/api/mess/add-listing")
def create_mess_listing(req: CreateMessListingRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] not in ["mess_partner", "admin"]:
        raise HTTPException(status_code=403, detail="Unauthorized action.")
        
    mess_id = f"mess-{uuid.uuid4().hex[:6]}"
    new_mess = DBMessListing(
        id=mess_id, name=req.name, provider_name=req.provider_name or user["full_name"],
        monthly_price=req.monthly_price, diet_type=req.diet_type, meals_per_day=req.meals_per_day,
        rating="5.0 (New)", address=req.address, google_map_url=req.google_map_url, description=req.description
    )
    db.add(new_mess)
    db.commit()
...
def delete_mess_listing(mess_id: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.delete(mess)
    db.commit()
    return {"status": "success", "message": "Mess listing deleted successfully."}

# ─── REMINDERS & PG/ROOMS ENDPOINTS ────────────────────────────────
...
def get_student_reminders(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if mess_student and mess_student.expiry_date:
        try:
            exp_dt = datetime.strptime(mess_student.expiry_date, "%Y-%m-%d").date()
            mess_days_left = max(0, (exp_dt - today).days)
    ...
    return {
        "mess_reminder": {
            ...
            "message": f"Your Mess Subscription expires in {mess_days_left} days ({mess_exp_date})."
        },
        "pg_reminder": {
            "has_booking": bool(booking),
            "days_left": pg_days_left, "due_date": pg_due_date,
            "should_alert": pg_days_left <= 3 and bool(booking),
...
def get_pgs(gender_pref: Optional[str] = Query(None), search: Optional[str] = Query(None), db: Session = Depends(get_db)):
    ...
    for l in query.all():
        ...
        result.append({
            ...
            "tag_label": l.tag_label, "address": l.address, "google_map_url": l.google_map_url,
            "rating": l.rating, "amenities": json.loads(l.amenities) if l.amenities else [], "images": pg_imgs
        })
    return result

@app.get("/api/rooms")
def get_rooms(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    rooms = db.query(DBPGRoom).all()
    is_privileged = user["role"] in ["pg_owner", "admin"]

    return [
        {
            "id": r.id, "pg_id": r.pg_id, "room_number": r.room_number, "room_type": r.room_type,
            "monthly_rent": r.monthly_rent, "status": r.status,
            "images": json.loads(r.images) if r.images else [],
            "tenant_name": r.tenant_name if is_privileged else ("Occupied" if r.status == "occupied" else "-"),
            "tenant_phone": r.tenant_phone if is_privileged else "-",
            "tenant_address": r.tenant_address if is_privileged else "-"
        }
        for r in rooms
    ]

@app.post("/api/pg/add-room")
def add_room(req: AddRoomRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] not in ["pg_owner", "admin"]:
        raise HTTPException(status_code=403, detail="Unauthorized action.")

    new_room = DBPGRoom(
        id=f"room-{uuid.uuid4().hex[:8]}", pg_id=req.pg_id, room_number=req.room_number,
        room_type=req.room_type, monthly_rent=req.monthly_rent, status="vacant", images="[]"
    )
    db.add(new_room)
    db.commit()
...
def update_room_images(req: UpdateRoomImagesRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    optimized_images = [compress_and_convert_to_webp(img) for img in req.images[:5]]
    room.images = json.dumps(optimized_images)
    db.commit()
    return {"status": "success", "message": f"Saved images for Room {req.room_number}!"}
...
def get_vacate_requests(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] not in ["pg_owner", "admin"]:
        raise HTTPException(status_code=403, detail="Unauthorized action.")
...
def approve_vacate_request(req: ApproveVacateRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] not in ["pg_owner", "admin"]:
        raise HTTPException(status_code=403, detail="Unauthorized action.")
...
# ─── PAYMENT & BOOKING ENDPOINTS (SECURED) ──────────────────────────
...
def create_payment_order(req: CreateOrderRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        if is_mess_item:
            ...
            if not mess:
                raise HTTPException(status_code=404, detail="Mess listing not found. Cannot process payment.")
            real_price = mess.monthly_price
            ...
            if not pg:
                raise HTTPException(status_code=404, detail="PG listing not found. Cannot process payment.")
            real_price = pg.monthly_price
            if pg.owner_id:
                owner = db.query(DBUser).filter(DBUser.id == pg.owner_id).first()
                if owner and owner.razorpay_account_id:
                    vendor_account_id = owner.razorpay_account_id

        total_paise = real_price * 100

        order_data = {
            "amount": total_paise,
            "currency": "INR",
            "receipt": f"rcpt_{uuid.uuid4().hex[:8]}",
            "notes": {"user_id": user["id"], "student_name": user["full_name"], "item_name": req.item_name}
        }
        ...
        if vendor_account_id:
            ...
            order_data["transfers"] = [{
                "account": vendor_account_id, "amount": vendor_payout_paise,
                "currency": "INR", "on_hold": 0
            }]

        order = razorpay_client.order.create(data=order_data)  # type: ignore
        return {"status": "success", "order_id": order["id"], "amount": order["amount"], "currency": order["currency"], "key_id": RAZORPAY_KEY_ID}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Order creation failed: {str(e)}")

@app.post("/api/payments/verify")
def verify_payment_and_fulfill(req: VerifyPaymentRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    gross = float(req.monthly_amount)
    platform_fee = round(gross * 0.05, 2)
    net_vendor_payout = round(gross - platform_fee, 2)

    db.add(DBPaymentReceipt(
        transaction_id=txn_id, order_id=req.razorpay_order_id, payer_name=user["full_name"],
        payer_phone=user["phone"] or user["email"], total_amount=gross, platform_fee=platform_fee,
        vendor_payout_amount=net_vendor_payout, payment_method="Razorpay (UPI/Card/NetBanking)",
        description=f"Paid for: {req.item_name}", payment_date=get_ist_now().strftime("%Y-%m-%d %H:%M:%S")
    ))
    ...
    db.add(DBBooking(
        id=booking_id, user_phone=user["phone"] or user["email"], target_type=target_type,
        item_name=req.item_name, move_in_date=req.move_in_date, special_requests=req.special_requests or "",
        monthly_amount=req.monthly_amount, payment_method="Razorpay", transaction_id=txn_id, status="Active"
    ))
    ...
    if is_mess_item:
        ...
        new_expiry = (get_ist_now() + timedelta(days=duration)).strftime("%Y-%m-%d")
        ...
        if ms:
            ...
            db.add(DBMessStudent(
                id=f"ms-{uuid.uuid4().hex[:6]}", name=user["full_name"], phone=user["phone"] or "9155118661",
                address=user["address"] or "Hostel", google_map_url=user["google_map_url"] or "",
                mess_name=extracted_mess_name, plan=req.item_name, diet=diet_choice,
                base_price=req.monthly_amount, is_active=True, expiry_date=new_expiry
            ))

        notify_owner_and_email(
            db=db, background_tasks=background_tasks, recipient_role="mess_partner",
            recipient_user_id=target_owner.id if target_owner else None,
            title="New Mess Subscription Confirmed",
            message=f"Student '{user['full_name']}' ({user['phone']}) subscribed to '{req.item_name}'. Address: {user['address']}"
        )
    else:
        ...
        notify_owner_and_email(
            db=db, background_tasks=background_tasks, recipient_role="pg_owner",
    ...
    db.commit()
    background_tasks.add_task(send_email_notification, user["email"], f"Payment Receipt: {req.item_name}", f"Hi {user['full_name']},\n\nPayment processed successfully! Txn ID: {txn_id}.")
...
# ─── CALENDAR, CANCELLATION & BILLING ENDPOINTS (SECURED) ─────────────
...
def cancel_meal(req: MealCancelRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Validate active mess subscription and expiry date
    mess_student = db.query(DBMessStudent).filter(DBMessStudent.name.ilike(user["full_name"])).first()
    if not mess_student or not mess_student.is_active:
        raise HTTPException(status_code=400, detail="You do not have an active mess subscription.")
    if mess_student.expiry_date and target_date > mess_student.expiry_date:
        raise HTTPException(status_code=400, detail="Cannot cancel meals past your subscription expiry date.")

    cutoffs = {"Breakfast": (7, 0, 30), "Lunch": (10, 0, 50), "Dinner": (18, 0, 50)}
    if req.meal_type not in cutoffs:
        raise HTTPException(status_code=400, detail="Invalid meal type.")

    cutoff_h, cutoff_m, refund_coins = cutoffs[req.meal_type]

    if target_date == today_str:
        cutoff_dt = ist_now.replace(hour=cutoff_h, minute=cutoff_m, second=0, microsecond=0)
        if ist_now > cutoff_dt:
            raise HTTPException(status_code=400, detail=f"Cutoff time for {req.meal_type} has passed for today.")
    ...
    new_cancel = DBMealCancellation(
        ...
        refund_amount=refund_coins, date=target_date, timestamp=ist_now.strftime("%I:%M %p")
    )
    db.add(new_cancel)
    db.commit()
...
def get_mess_daily_stats(date: str = Query(default=lambda: get_ist_now().strftime("%Y-%m-%d")), authorization: Optional[str] = Header(None), db: Session = Depends(get_db)):
    ...
    if authorization and authorization.startswith("Bearer "):
        ...
        if user and user.role == "mess_partner":
            ...
            query_students = query_students.filter(or_(DBMessStudent.mess_name.ilike(f"%{target_mess_name}%"), DBMessStudent.plan.ilike(f"%{target_mess_name}%")))
    ...
    return {
        "date": date, "total_enrolled": len(active_students), "total_cancellations": len(cancels),
        ...
        "cancelled_orders_list": [{"id": c.id, "student_name": c.student_name, "phone": c.phone, "meal": c.meal, "refund": c.refund_amount, "timestamp": c.timestamp} for c in cancels],
        "enrolled_students": [{"id": s.id, "name": s.name, "phone": s.phone, "address": s.address, "google_map_url": s.google_map_url, "plan": s.plan, "diet": s.diet, "is_active": s.is_active} for s in active_students]
    }

@app.get("/api/mess/owner-monthly-billing")
def get_owner_monthly_billing(month: str = Query(default="2026-09"), authorization: Optional[str] = Header(None), db: Session = Depends(get_db)):
    ...
    if authorization and authorization.startswith("Bearer "):
        ...
        if user and user.role == "mess_partner":
            mess = db.query(DBMessListing).filter(DBMessListing.provider_name.ilike(f"%{user.full_name}%")).first()
            target_mess_name = mess.name if mess else user.full_name
            query_subscribers = query_subscribers.filter(or_(DBMessStudent.mess_name.ilike(f"%{target_mess_name}%"), DBMessStudent.plan.ilike(f"%{target_mess_name}%")))

    subscribers = query_subscribers.all()
    billing_data = []

    for s in subscribers:
        cancels = db.query(DBMealCancellation).filter(DBMealCancellation.student_name == s.name, DBMealCancellation.date.like(f"{month}%")).all()
        ...
        billing_data.append({
            ...
            "base_price": s.base_price if s.is_active else 0, "total_cancellations": len(cancels),
            "total_refund_amount": total_refund, "final_verified_bill": max(0, s.base_price - total_refund) if s.is_active else 0
        })

    return {"month": month, "subscribers": billing_data}

@app.get("/api/mess/student-calendar")
def get_student_calendar(month: str = Query(default="2026-09"), student_name: Optional[str] = Query(None), user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    cancels = db.query(DBMealCancellation).filter(DBMealCancellation.student_name == target_name, DBMealCancellation.date.like(f"{month}%")).all()
    cancel_map = {}
    total_refund = 0
    for c in cancels:
        try:
...
def update_weekly_menu(req: WeeklyMenuUpdateRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] not in ["mess_partner", "admin"]:
        raise HTTPException(status_code=403, detail="Unauthorized action.")
...
def get_vendor_monthly_credits(month: str = "2026-09", user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] not in ["mess_partner", "pg_owner", "admin"]:
        raise HTTPException(status_code=403, detail="Unauthorized.")

    receipts = db.query(DBPaymentReceipt).filter(DBPaymentReceipt.payment_date.like(f"{month}%")).all()
    ...
    for r in receipts:
        ...
        total_net += r.vendor_payout_amount
        fee = r.total_amount - r.vendor_payout_amount
        total_fee += fee

        txns.append({
            "date": r.payment_date, "student_name": r.payer_name, "student_phone": r.payer_phone,
            "description": r.description, "gross_amount": r.total_amount, "net_credit": r.vendor_payout_amount,
            "status": "Settled"
        })

    return {
        "status": "success", "month": month,
        "summary": {"total_gross": round(total_gross, 2), "total_net_payout": round(total_net, 2), "total_commission": round(total_fee, 2)},
        "transactions": txns
    }

@app.post("/api/vendor/onboard-route")
def onboard_vendor_razorpay_route(req: VendorRouteOnboardRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] not in ["mess_partner", "pg_owner", "admin"]:
    ...
    if db_user:
        db_user.bank_account_no = req.bank_account_no.strip()
        db_user.bank_ifsc = req.bank_ifsc.strip().upper()
        db_user.account_holder_name = req.account_holder_name.strip()
        db_user.pan_number = req.pan_number.strip().upper()
        db_user.razorpay_account_id = f"acc_linked_{uuid.uuid4().hex[:8]}"
        db.commit()
    return {"status": "success", "message": "Bank details saved and direct payouts activated!"}

# ─── COMPLAINTS & NOTIFICATIONS ENDPOINTS ──────────────────────────
...
def raise_complaint(req: ComplaintCreateRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
def resolve_complaint(req: ResolveComplaintRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
def get_admin_metrics(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] != "admin":
        raise HTTPException(status_code=403, detail="Unauthorized.")
    return {
...
def get_admin_users(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] != "admin":
        raise HTTPException(status_code=403, detail="Unauthorized.")
...
def get_email_status(user: dict = Depends(get_current_user)):
    ...
    return {"status": "success", "recent_logs": EMAIL_AUDIT_LOG}

@app.post("/api/admin/send-test-email")
def send_admin_test_email(req: TestEmailRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user)):
    if user["role"] != "admin":
        raise HTTPException(status_code=403, detail="Admin authorization required.")
    background_tasks.add_task(send_email_notification, req.recipient, req.subject or "Basera Test Email", req.body or "Test email body.")
    return {"status": "success", "message": f"Test email dispatched to {req.recipient}."}

@app.get("/api/admin/email-logs")
def get_admin_email_logs(user: dict = Depends(get_current_user)):
    if user["role"] != "admin":
        raise HTTPException(status_code=403, detail="Unauthorized.")
...
def create_admin_pg_listing(req: CreatePGRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user["role"] != "admin":
        raise HTTPException(status_code=403, detail="Unauthorized.")
    new_pg = DBPGListing(
        id=f"pg-{uuid.uuid4().hex[:6]}", owner_id=user["id"], name=req.name,
        distance_km=req.distance_km, gender_pref=req.gender_pref, sharing=req.sharing,
        monthly_price=req.monthly_price, tag_label="Verified PG", address=req.address,
        google_map_url="https://maps.google.com/?q=Bokaro", rating=req.rating,
        amenities=req.amenities, images="[]"
    )
    db.add(new_pg)
    db.commit()
